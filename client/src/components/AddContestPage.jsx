...
const AddContestPage = () => {
    ...
    useEffect(() => {
        const fetchProblems = async () => {
            try {
                // Fetching all problems from the admin-only route
                const response = await fetch(`${API_BASE_URL}/problems/admin`, {
                    credentials: 'include'
                });
        ...
        if (!loadingAuth && isAdmin) {
            ...
            navigate('/contests');
            showNotification('You must be an administrator to add a contest.', 'error');
        }
    }, [isAuthenticated, currentUser, loadingAuth, isAdmin, navigate]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showNotification('Contest created successfully!');
            setTimeout(() => navigate('/contests'), 3000);
        } catch (err) {
            setError(err.message);
            showNotification(`Error creating contest: ${err.message}`, 'error');
            setLoading(false);
        }
    };
        ...
        return (
            <div className="flex flex-col min-h-screen p-8 pt-24 bg-gradient-to-br from-white to-gray-100 text-gray-900 dark:from-gray-900 dark:to-black dark:text-white items-center justify-center">
                <p className="text-xl dark:text-white">
                    {loadingAuth ? 'Checking administrator access...' : 'Loading problems...'}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col min-h-screen p-8 pt-24 bg-gradient-to-br from-white to-gray-100 text-gray-900 dark:from-gray-900 dark:to-black dark:text-white">
            <div className="container mx-auto bg-white dark:bg-gray-800 p-10 rounded-lg shadow-2xl w-full max-w-5xl my-10 border border-gray-200 dark:border-gray-700">
                <h2 className="text-4xl font-extrabold text-center mb-10 text-blue-600 dark:text-blue-400">Add New Contest</h2>
